...
// Proper Agora remote video player — uses useEffect to call play() after DOM mount
...
const LocalVideoPlayer = ({ videoTrack, label }) => {
  ...
  return (
    <div className="w-full h-full bg-[#111114] rounded-3xl overflow-hidden border-2 border-white/20 shadow-2xl relative">
      <div ref={containerRef} className="w-full h-full" />
      <div className="absolute bottom-3 left-3 flex items-center gap-2 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10">
        ...
        <span className="text-white font-black text-[8px] uppercase tracking-wider">{label}</span>
      </div>
    </div>
  );
};
  ...
  if (inCall) {
    return (
      <div className="fixed inset-0 bg-[#0a0a0c] z-50 flex flex-col font-sans overflow-hidden">
          ...
          <div className="flex items-center justify-between px-10 py-6 border-b border-white/5 bg-black/40 backdrop-blur-xl shrink-0">
            ...
            <div className="flex items-center gap-4">
              {activeConv?.paymentStatus === 'held' && (
                ...
                >
                  {releasing ? 'Requesting...' : 'Request Completion'}
                </button>
              )}
          ...
          <div className={`relative transition-all duration-700 ease-in-out ${isFullScreen ? 'w-full h-full p-0' : 'w-full max-w-6xl aspect-video rounded-[40px] shadow-2xl shadow-indigo-500/10 overflow-hidden'}`}>
            {remoteUsers.length > 0 ? (
              <RemoteVideoPlayer
                ...
                label={getOtherUser(activeConv)?.name || 'Learner'}
              />
            ) : (
              <div className="w-full h-full bg-[#111114] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-white/20 font-black text-sm uppercase tracking-widest">Waiting for learner...</p>
              </div>
            )}
        ...
        {/* Floating Controls Overlay (Visible when full screen on hover or always when not) */}
        <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/60 backdrop-blur-2xl border border-white/10 px-8 py-5 rounded-[2.5rem] z-30 transition-all ${isFullScreen ? 'opacity-0 hover:opacity-100' : 'opacity-100'}`}>
           ...
           {activeConv?.paymentStatus === 'held' && isFullScreen && (
              <button 
                onClick={handleCompleteCall} 
                disabled={releasing} 
                ...
                <div className="flex items-center gap-3">
                  {activeConv.paymentStatus === 'held' && (
                    <button 
                      onClick={handleCompleteCall} 
                      disabled={releasing}
                      className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-2xl font-black text-xs transition-all uppercase tracking-widest shadow-xl shadow-emerald-500/20 active:scale-95 disabled:opacity-50"
                    >
                      {releasing ? 'Requesting...' : 'Complete Session'}
                    </button>
                  )}
                  {activeConv.paymentStatus === 'completion_requested' && (
                    <span className="bg-amber-100 text-amber-700 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest border border-amber-200">
                      Awaiting Approval
                    </span>
                  )}
                  <button onClick={() => navigate('/tutor/video', { state: { session: activeConv } })}
